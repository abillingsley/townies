...
import {
  BaseEntity, BaseError, IFindOptions, IMapper, IRepository, RepositoryId,
  UniqueConstraintError, ValidationError,
} from "~/core";
import BaseModel from "./models/baseModel";

@injectable()
export class SequelizeRepository<T extends BaseEntity, U extends BaseModel<U>>
  ...
  public async list(options?: IFindOptions<T>): Promise<T[]> {
    return this.dbContext.findAll(options as ISequelizeFindOptions)
  ...
  public async add(entity: T, options: any = {}): Promise<T> {
    return this.dbContext.create(entity, options)
  ...
  public async update(entity: T): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this.dbContext.update(entity, { where: { id: entity.id } })
  ...
  public async delete(entity: T): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this.dbContext.destroy({ where: { id: entity.id } }).then((result: number) => {
        if (result > 0) {
          resolve();
        } else {
          reject();
